...
public class UnitTest {
    ...
    public void unitTest() throws Exception {
        final String path = "src/main/resources/case/TestData.json";
        String result = "";

        final String url = GetData.get_api_data(path).get("domain").toString() // 获取域名
                + ":" + GetData.get_api_data(path).get("port").toString() // 获取端口号
                + GetData.get_api_data(path).get("api").toString(); // 获取接口名

        final String body = GetData.get_api_data(path).get("body").toString(); // 获取body
